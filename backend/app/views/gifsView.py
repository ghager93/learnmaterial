...
mod = Blueprint("gifsView", __name__)
...
def get(id):
    ...
    try:
        row = db.session.query(GifsModel).filter_by(id=id).first()
    except exc.InvalidRequestError as e:
        return (
            jsonify({"error": {"code": 400, "message": "Invalid query parameter"}}),
            400,
...
def create():
    ...
    validation_result = GifsValidation.validate(request.get_json())
